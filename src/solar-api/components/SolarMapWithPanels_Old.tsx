...
const SolarMapWithPanels = ({ 
  ...
  useEffect(() => {
    const initializeMap = async () => {
      ...
      try {
        ...
        // Create the map with maximum zoom for detailed solar analysis
        const googleMap = new google.maps.Map(mapRef.current, {
          center: { lat: center.lat, lng: center.lng },
          zoom: 23, // Maximum zoom for ultra-detailed satellite imagery
  ...
  useEffect(() => {
    ...
    const panelsToShow = getDistributedPanelLocations(buildingData, panelCount);
        ...
        icon: {
          url: 'data:image/svg+xml;base64,' + btoa(`
            <svg width=\"${panelSize}\" height=\"${Math.round(panelSize/2)}\" viewBox=\"0 0 ${panelSize} ${Math.round(panelSize/2)}\" xmlns=\"http://www.w3.org/2000/svg\">
              <!-- Solar panel background with realistic proportions (2:1 ratio) -->
              <rect x=\"1\" y=\"1\" width=\"${panelSize-2}\" height=\"${Math.round(panelSize/2)-2}\" rx=\"1\" 
                    fill=\"${panelColor}\" stroke=\"white\" stroke-width=\"1\" opacity=\"0.95\"/>
              ...
              <circle cx=\"3\" cy=\"3\" r=\"1\" fill=\"#666\" opacity=\"0.7\"/>
              <circle cx=\"${panelSize-3}\" cy=\"3\" r=\"1\" fill=\"#666\" opacity=\"0.7\"/>
              <circle cx=\"3\" cy=\"${Math.round(panelSize/2)-3}\" r=\"1\" fill=\"#666\" opacity=\"0.7\"/>
              <circle cx=\"${panelSize-3}\" cy=\"${Math.round(panelSize/2)-3}\" r=\"1\" fill=\"#666\" opacity=\"0.7\"/>
            </svg>
          `),
...
// Legacy function - replaced by improved algorithm
// function calculateLiveData(...) { ... }

// Legacy functions removed - now using improved panel placement algorithm from ImprovedPanelPlacement.tsx

// Helper function to get panel icon size based on zoom level
function getPanelIconSize(zoomLevel: number): number {
  if (zoomLevel >= 21) return 40; // Very detailed view
  if (zoomLevel >= 20) return 32; // Large detail view
  if (zoomLevel >= 19) return 28; // Standard view  
  if (zoomLevel >= 18) return 24; // Medium view
  if (zoomLevel >= 17) return 20; // Small view
  return 16; // Overview
}
